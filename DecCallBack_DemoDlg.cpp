...
{	
	...
	unsigned char *ptrRGB;//初始时指针指向位置
//	unsigned char *ptrR, *ptrG, *ptrB;
	ptrY = yuvBuf;
	ptrU = yuvBuf + w * h;
	ptrUV = yuvBuf + w * h;
//	ptrV = yuvBuf + w * h *3/4;//注意YUV420的储存格式为planar
	ptrRGB = rgbBuf;//注意RGB24的储存格式为packed
//	ptrR = rgbBuf;
//	ptrG = rgbBuf + w * h;
//	ptrB = rgbBuf + w * h * 2;
	unsigned char y, u, v, r, g, b;//rgb取值范围0~255
	for (int j = 0; j < h; j++)
	{
		for (int i = 0; i < w; i++)
		{	
			y = *(ptrY++);
			if (j % 2 == 0 && i % 2 == 0)//每4个Y公用一个U或V
			{
				//u = *(ptrU++);
				//v = *(ptrV++);
				u = *(ptrUV++);
...
void funproc(char *pBuf)
{
	...
	yuv2rgb(pBuf, WIDTH, HEIGTH, rgb24Buf);//yuv转换成rgb
...
/*
		...
		int w = 2048, h = 1536;
		unsigned char *rgb24Buf = (unsigned char *)malloc(w*h*3);//在堆中申请一块区域用于存放计算出的rgb
		yuv2rgb(pBuf, w, h, rgb24Buf);
		fwrite(rgb24Buf, w*h * 3, 1, rgbFile);
		free(rgb24Buf);//释放申请的空间	*/
...
void CDecCallBack_DemoDlg::OnOK() 
{
	...
	if (llRealHandle<0)
	{
		...
		//创建显示窗口
		cvNamedWindow("a");
		rgb24Buf = (unsigned char *)malloc(WIDTH*HEIGTH * 3);//在堆中申请一块区域用于存放计算出的rgb
	}
	else
	{	
		//停止预览
